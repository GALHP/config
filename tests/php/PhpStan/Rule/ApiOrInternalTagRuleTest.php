...
final class ApiOrInternalTagRuleTest extends RuleTestCase
{
    public function testRule(): void
    {
        $this->analyse([__DIR__ . '/../../Fixtures/ApiOrInternalTagRuleFixture.php'], [
            ...
            [sprintf('Constant `%s` must be annotated with either @internal or @api.', 'CONSTANT_WITHOUT_TAG_B'), 42],
            [sprintf('Interface `%s` must be annotated with either @internal or @api.', 'InterfaceWithoutTag'), 56],
            [sprintf('Enum `%s` must be annotated with either @internal or @api.', 'EnumWithoutTag'), 68],
            [sprintf('Trait `%s` must be annotated with either @internal or @api.', 'TraitWithoutTag'), 80],
        ]);
    }
